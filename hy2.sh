...
选项:
    -p, --port PORT   设置监听端口 (默认: 29999)
    --service         安装为 systemd 服务
    -h, --help        显示此帮助信息

示例:
    $0 -p 30000
    $0 -p 30000 --service
    sudo $0 -p 30000 --service
...
if [[ "$INSTALL_AS_SERVICE" == true ]] && [[ $EUID -ne 0 ]]; then
    ...
    error "安装服务需要 root 权限，请使用 sudo 运行"
...
write_config() {
    ...
    declare -g AUTH_PASSWORD  # 声明为全局变量
